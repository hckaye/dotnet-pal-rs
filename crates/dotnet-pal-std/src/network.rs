...
mod unix {
    ...
    unsafe fn prefix(mask: *const libc::sockaddr, family: i32, host: u32) -> u32 {
        ...
        #[cfg(any(target_vendor = "apple", target_os = "freebsd", target_os = "netbsd", target_os = "openbsd", target_os = "dragonfly"))]
    ...
    #[cfg(target_vendor = "apple")]
    unsafe fn link(entry: &libc::ifaddrs, _: &[u8], interface: &mut Interface) {
        /// The start of `struct if_data` of <net/if_var.h>, up to the line speed.
        #[repr(C)]
        struct Data { kinds: [u8; 8], mtu: u32, metric: u32, baudrate: u32 }
        if unsafe { (*entry.ifa_addr).sa_family } as i32 != libc::AF_LINK { return; }
        let link = unsafe { entry.ifa_addr.cast::<libc::sockaddr_dl>().read_unaligned() };
        // IFT_LOOP, IFT_ETHER, IFT_IEEE80211, IFT_PPP, IFT_GIF and IFT_STF of <net/if_types.h>.
        interface.kind = match link.sdl_type {
            0x18 => network::INTERFACE_LOOPBACK, 0x06 => network::INTERFACE_ETHERNET, 0x47 => network::INTERFACE_WIRELESS,
            0x17 => INTERFACE_POINT_TO_POINT, 0x37 | 0x39 => network::INTERFACE_TUNNEL,
            _ => interface.kind,
        };
        if let Some(data) = unsafe { entry.ifa_data.cast::<Data>().as_ref() } { if data.baudrate != u32::MAX { interface.speed_bps = data.baudrate as u64; } }
        // The address follows the name inside sdl_data, which runs to sdl_len and may be longer than the declared array.
        let (start, length) = (std::mem::offset_of!(libc::sockaddr_dl, sdl_data) + link.sdl_nlen as usize, link.sdl_alen as usize);
        if length <= interface.hardware_address.len() && start + length <= link.sdl_len as usize {
            interface.hardware_address_length = length as u32;
            interface.hardware_address[..length].copy_from_slice(unsafe { std::slice::from_raw_parts(entry.ifa_addr.cast::<u8>().add(start), length) });
        }
    }
    #[cfg(not(any(target_os = "linux", target_os = "android", target_vendor = "apple")))]
    unsafe fn link(_: &libc::ifaddrs, _: &[u8], _: &mut Interface) {}
