...
mod unix {
    ...
    fn user(socket: &Socket) -> Result<u32> {
        let (mut user, mut group) = (u32::MAX, u32::MAX);
        if unsafe { libc::getpeereid(std::os::fd::AsRawFd::as_raw_fd(&socket.inner), &mut user, &mut group) } != 0 { return Err(error(io::Error::last_os_error())); }
        Ok(user)
    }
    ...
    fn user(_: &Socket) -> Result<u32> { Err(Error::Unsupported) }
