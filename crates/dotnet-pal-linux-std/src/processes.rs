...
struct Process { state: Mutex<State>, changed: Condvar,  id: u32, }
struct Pipe(fs::File);
...
unsafe fn start(program: &[u8], arguments: &[*const u8], environment: Option<&[*const u8]>, directory: Option<&[u8]>, pipes: u32,
    ...
    let process = Process { state: Mutex::new(State { child, fate: Fate::Running, watched: false }), changed: Condvar::new(),  id, };
