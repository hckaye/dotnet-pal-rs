...
use dotnet_pal_rs::sockets::{self, Address, PollEntry, DATAGRAM, IPV4, IPV6, LOCAL, MAX_HOST_NAME, POLL_CHANNELS, POLL_ERROR, POLL_HANGUP, POLL_READ, POLL_WRITE, RAW, RECEIVE_PEEK, STREAM};
...
impl port::Sockets for Linux {
    ...
    unsafe fn get_option(socket: *mut c_void, option: u32) -> Result<u64> {
        ...
        match option {
            ...
            sockets::ERROR => Ok(match get::<i32>(fd, libc::SOL_SOCKET, libc::SO_ERROR)? { 0 => dotnet_pal_rs::OK, code => { forget_errors(fd); error(code).status() } } as u64),
            sockets::AVAILABLE => {
                let mut value: i32 = 0;
