...
pub(crate) unsafe fn opened<'a>(file: *mut c_void) -> Result<(&'a fs::File, u32)> { unsafe { borrow::<OpenFile>(file) }.map(|open| (&open.file, open.flags)) }
...
fn error(e: io::Error) -> Error {
    ...
    if let Some(code) = e.raw_os_error() {
        return match code {
            ...
            libc::ENOMEM => Error::OutOfMemory, libc::EINVAL => Error::InvalidArgument, libc::EBUSY => Error::Busy,
            _ => Error::Os,
        };
    }
...
fn describe(meta: &fs::Metadata) -> Status {
    ...
    let (mode, changed_ns, identity, device) = {
        use std::os::unix::fs::MetadataExt;
        let changed = meta.ctime() as i128 * 1_000_000_000 + meta.ctime_nsec() as i128;
        (meta.mode() & 0o7777, u64::try_from(changed).unwrap_or(0), meta.ino(), meta.dev())
    };
...
fn permissions(mode: u32, _current: impl FnOnce() -> io::Result<fs::Metadata>) -> Result<fs::Permissions> {
    Ok(std::os::unix::fs::PermissionsExt::from_mode(mode))
}
...
fn set_path_times(path: &[u8], follow: bool, accessed_ns: Option<u64>, modified_ns: Option<u64>) -> Result<()> {
    let moment = |ns: Option<u64>| -> Result<libc::timespec> {
        let mut value: libc::timespec = unsafe { std::mem::zeroed() }; // some targets pad the struct
        match ns {
    ...
    let (path, times) = (std::ffi::CString::new(path).map_err(|_| Error::InvalidArgument)?, [moment(accessed_ns)?, moment(modified_ns)?]);
    let flags = if follow { 0 } else { libc::AT_SYMLINK_NOFOLLOW };
    retry(|| if unsafe { libc::utimensat(libc::AT_FDCWD, path.as_ptr(), times.as_ptr(), flags) } == 0 { Ok(()) } else { Err(io::Error::last_os_error()) })
}
...
fn lock_range(file: &fs::File, offset: u64, length: u64, mode: u32) -> Result<()> {
    let mut range: libc::flock = unsafe { std::mem::zeroed() }; // l_pid stays zero, as the command demands
    range.l_type = match mode { files::LOCK_SHARED => libc::F_RDLCK, files::LOCK_EXCLUSIVE => libc::F_WRLCK, files::LOCK_UNLOCK => libc::F_UNLCK, _ => return Err(Error::InvalidArgument) } as _;
    range.l_whence = libc::SEEK_SET as _;
    range.l_start = offset.try_into().map_err(|_| Error::InvalidArgument)?;
    range.l_len = length.try_into().map_err(|_| Error::InvalidArgument)?;
    loop {
        if unsafe { libc::fcntl(std::os::fd::AsRawFd::as_raw_fd(file), libc::F_OFD_SETLK, &range) } == 0 { return Ok(()); }
        let e = io::Error::last_os_error();
        return Err(match e.raw_os_error() {
            Some(libc::EINTR) => continue,
            Some(libc::EAGAIN) | Some(libc::EACCES) => Error::WouldBlock,
            // The descriptor is alive: a shared range needs read access and an exclusive one write access.
            Some(libc::EBADF) => Error::AccessDenied,
            // Every argument is valid here, so this is a kernel older than the command.
            Some(libc::EINVAL) => Error::Unsupported,
            _ => error(e),
        });
    }
}
...
impl port::Files for Std {
    unsafe fn open(path: &[u8], flags: u32, mode: u32) -> Result<*mut c_void> {
...

        {
            ...
            options.mode(mode);
            if create && !write { options.custom_flags(libc::O_CREAT | if exclusive { libc::O_EXCL } else { 0 }); }
        }
    ...
    unsafe fn create_directory(path: &[u8], mode: u32) -> Result<()> {
        ...
        let result = { use std::os::unix::fs::DirBuilderExt; fs::DirBuilder::new().mode(mode).create(path) };
    ...
    unsafe fn symlink(target: &[u8], created: &[u8]) -> Result<()> {
        ...
        let result = std::os::unix::fs::symlink(target, created);
