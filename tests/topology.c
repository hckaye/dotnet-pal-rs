...
int main(int argc, char **argv) {
    ...
    if (pal_topology_fault == 2) {
        ...
        assert(t->read_stats(&stats, sizeof stats) == 0 && stats.rejected_or_failed >= 4 && stats.cpu_ok == 0);
    ...
    dotnet_pal_topology_stats stats;
    assert(t->read_stats(&stats, sizeof stats) == 0 && stats.cpu_ok >= 3 && stats.memory_ok >= 3 && stats.affinity_ok >= 2);
    assert(t->read_stats(&stats, sizeof stats - 1) == DOTNET_PAL_INVALID_ARGUMENT);
    printf("TOPOLOGY PASS max=%u count=%u affinity=%u total=%llu available=%llu limit=%llu cache=%zu\n",
           max, count, affinity, (unsigned long long)total, (unsigned long long)available, (unsigned long long)limit, cache);
    return 0;
}
