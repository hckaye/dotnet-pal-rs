...
def main():
    ...
    subprocess.run(["git", "-C", str(root), "diff", "--exit-code", "HEAD", "--", GC_FILE, CMAKE_FILE, *kernel_patch.FILES, *context_patch.FILES, *support_patch.FILES, *images_patch.FILES], check=True)
    ...
    extra[kernel_patch.PAL] = images_patch.pal(extra[kernel_patch.PAL])
    extra[support_patch.DUMP] = images_patch.dump(extra[support_patch.DUMP])
    extra.update({path: transform((root / path).read_text()) for path, transform in images_patch.TRANSFORMS.items()})
...
if(DOTNET_PAL_ROOT)
  ...
  add_definitions(-DDOTNET_PAL_GC_VM=1 -DDOTNET_PAL_KERNEL=1 -DDOTNET_PAL_RUNTIME=1 -DDOTNET_PAL_NATIVE_CONTEXT=1 -DDOTNET_PAL_SUPPORT=1 -DDOTNET_PAL_IMAGES=1)
