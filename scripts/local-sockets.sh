...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/local_sockets.c "$root/release/libdotnet_pal_rs.a" \
...
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-sockets,host-local-sockets --target-dir "$root/host-local-sockets"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/local_sockets.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/sockets_host.c tests/local_sockets_host.c "$root/host-local-sockets/release/libdotnet_pal_rs.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/local-sockets/local-sockets-host
