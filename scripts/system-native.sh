...
# artifacts/system-native/libSystem.Native.a from crates/dotnet-pal-build/native/system_native_{pal,io,net,sys,proc}.c.
...
for unit in pal io net sys proc; do
  clang -std=c11 -O2 -fPIC -ftls-model=initial-exec -ffunction-sections -fdata-sections -Wall -Wextra -Werror -Iinclude -Icrates/dotnet-pal-build/native \
    -c "crates/dotnet-pal-build/native/system_native_$unit.c" -o "artifacts/system-native/system_native_$unit.o"
