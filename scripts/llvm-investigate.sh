#!/usr/bin/env bash
# Baseline experiment: official NativeAOT LLVM, no Rust GC redirection yet.
set -euo pipefail
cd "$(dirname "$0")/.."
...
dotnet publish "$project" -r wasi-wasm -c Release -o artifacts/llvm/publish 2>&1 | tee artifacts/llvm/build.log
