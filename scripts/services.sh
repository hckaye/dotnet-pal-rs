...
cc "${common[@]}" tests/services.c target/release/libdotnet_pal_rs.a "${libs[@]}" -o artifacts/services-linux
timeout 60s artifacts/services-linux
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-services --target-dir target/host-services
cc "${common[@]}" -DPAL_SERVICES_FAULT_HOST tests/services.c tests/host_backend.c tests/services_host.c \
  target/host-services/release/libdotnet_pal_rs.a "${libs[@]}" -o artifacts/services-host
...
cc "${common[@]}" tests/services.c tests/host_backend.c target/host/release/libdotnet_pal_rs.a "${libs[@]}" -o artifacts/services-legacy-host
...
cc "${common[@]}" tests/services.c target/linear/release/libdotnet_pal_rs.a "${libs[@]}" -o artifacts/services-none
timeout 60s artifacts/services-none
c++ -std=c++17 -Wall -Wextra -Werror -Iinclude -Inative tests/services_adapter.cpp -o artifacts/services-adapter
