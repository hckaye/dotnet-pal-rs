...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/mappings.c "$root/release/libdotnet_pal_standalone.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/mappings/mappings-linux
timeout 60s artifacts/mappings/mappings-linux
cargo rustc -p dotnet-pal-standalone --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-files,host-mappings --target-dir "$root/host-mappings"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/mappings.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/files_host.c tests/mappings_host.c "$root/host-mappings/release/libdotnet_pal_standalone.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/mappings/mappings-host
