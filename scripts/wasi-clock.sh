...
cargo build --release --no-default-features --features wasi-clock --target wasm32-wasip1 --target-dir target/wasi-clock
"${CLANG:-clang}" --target=wasm32-wasi -std=c11 -O2 -ffreestanding -fno-builtin \
  -Wall -Wextra -Werror -Iinclude -nostdlib tests/wasi_clock.c tests/freestanding_memory.c \
  target/wasi-clock/wasm32-wasip1/release/libdotnet_pal_rs.a \
