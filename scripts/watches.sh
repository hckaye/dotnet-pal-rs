...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/watches.c "$root/release/libdotnet_pal_rs.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/watches/watches-linux
timeout 60s artifacts/watches/watches-linux
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-watches --target-dir "$root/host-watches"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/watches.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/watches_host.c "$root/host-watches/release/libdotnet_pal_rs.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/watches/watches-host
