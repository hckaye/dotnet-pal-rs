...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/processes.c "$root/release/libdotnet_pal_standalone.a" \
...
cargo rustc -p dotnet-pal-standalone --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-processes --target-dir "$root/host-processes"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/processes.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/processes_host.c "$root/host-processes/release/libdotnet_pal_standalone.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/processes/processes-host
