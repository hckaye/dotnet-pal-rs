...
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-faults --target-dir target/host-faults
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/faults.c tests/faults_host.c tests/host_backend.c \
  target/host-faults/release/libdotnet_pal_rs.a -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/faults/host
