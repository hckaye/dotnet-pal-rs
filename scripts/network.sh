...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/network.c "$root/release/libdotnet_pal_rs.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/network/network-linux
timeout 60s artifacts/network/network-linux
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-sockets,host-network --target-dir "$root/host-network"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/network.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/sockets_host.c tests/network_host.c "$root/host-network/release/libdotnet_pal_rs.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/network/network-host
