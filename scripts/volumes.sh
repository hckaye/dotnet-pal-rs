...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/volumes.c "$root/release/libdotnet_pal_rs.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/volumes/volumes-linux
timeout 60s artifacts/volumes/volumes-linux
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-volumes --target-dir "$root/host-volumes"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/volumes.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/volumes_host.c "$root/host-volumes/release/libdotnet_pal_rs.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/volumes/volumes-host
