...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/sockets.c "$root/release/libdotnet_pal_rs.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/sockets/sockets-linux
timeout 60s artifacts/sockets/sockets-linux
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-sockets --target-dir "$root/host-sockets"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/sockets.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/sockets_host.c "$root/host-sockets/release/libdotnet_pal_rs.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/sockets/sockets-host
