...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/spawn_as.c "$root/release/libdotnet_pal_standalone.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/spawn-as/spawn-as-linux
timeout 120s artifacts/spawn-as/spawn-as-linux
cargo rustc -p dotnet-pal-standalone --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-processes,host-spawn-as --target-dir "$root/host-spawn-as"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/spawn_as.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/processes_host.c tests/spawn_as_host.c "$root/host-spawn-as/release/libdotnet_pal_standalone.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/spawn-as/spawn-as-host
