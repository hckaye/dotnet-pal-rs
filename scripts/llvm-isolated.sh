...
MSBuildEnableWorkloadResolver=false dotnet publish samples/LlvmGcProbe/LlvmGcProbe.csproj   -r wasi-wasm -c Release -p:IlcLlvmTarget=wasm32-unknown-wasip1 -p:PalWrap=false   "-p:IlcSdkPath=$root/artifacts/llvm/source-sdk/" "-p:PalSourceManifest=$root/artifacts/llvm/source-manifest.json"   "-p:PalObserverObject=$root/artifacts/llvm/baseline.o" "-p:PalBridgeObject=$root/artifacts/llvm/wasi_bridge.o"   "-p:PalLib=$root/target/wasi-isolated/wasm32-wasip1/release/libdotnet_pal_rs.a" -o artifacts/llvm/isolated   2>&1 | tee artifacts/llvm/isolated-publish.log
