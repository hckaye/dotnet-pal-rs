...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/notifications.c "$root/release/libdotnet_pal_rs.a" \
...
cargo rustc --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-notifications --target-dir "$root/host-notifications"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/notifications.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/notifications_host.c "$root/host-notifications/release/libdotnet_pal_rs.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/notifications/notifications-host
