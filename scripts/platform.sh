...
for group in topology process image streams; do
  cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude "tests/$group.c" target/release/libdotnet_pal_rs.a \
...
cargo rustc --lib --crate-type staticlib --release --no-default-features \
...
for group in topology process image streams; do
  ...
  cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST "tests/$group.c" $providers \
    target/host-platform/release/libdotnet_pal_rs.a -Wl,--gc-sections -Wl,--build-id=sha1 -lpthread -ldl -lm -o "artifacts/platform/$group-host"
