...
for target in wasm32-unknown-unknown wasm32-wasip1 wasm32v1-none; do
  cargo rustc -p dotnet-pal-standalone --lib --crate-type staticlib --release --no-default-features --features linear --target "$target"
  ctarget=wasm32-unknown-unknown
  [[ "$target" != wasm32-wasip1 ]] || ctarget=wasm32-wasip1
  extra=()
  [[ "$target" != wasm32v1-none ]] || extra=(-mcpu=mvp)
  "${CLANG:-clang}" --target="$ctarget" ${extra[@]+"${extra[@]}"} -std=c11 -O2 -ffreestanding -fno-builtin \
    -Wall -Wextra -Werror -Iinclude -nostdlib tests/linear.c tests/freestanding_memory.c \
    "target/$target/release/libdotnet_pal_standalone.a" -Wl,--no-entry -Wl,--export=pal_test \
