...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/accounts.c "$root/release/libdotnet_pal_standalone.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/accounts/accounts-linux
timeout 60s artifacts/accounts/accounts-linux
cargo rustc -p dotnet-pal-standalone --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-accounts --target-dir "$root/host-accounts"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/accounts.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/accounts_host.c "$root/host-accounts/release/libdotnet_pal_standalone.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/accounts/accounts-host
