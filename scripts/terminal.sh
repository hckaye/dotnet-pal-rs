...
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude tests/terminal.c "$root/release/libdotnet_pal_standalone.a" \
  -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/terminal/terminal-linux
timeout 60s artifacts/terminal/terminal-linux
cargo rustc -p dotnet-pal-standalone --lib --crate-type staticlib --release --no-default-features --features host-runtime,host-terminal --target-dir "$root/host-terminal"
cc -std=c11 -O2 -Wall -Wextra -Werror -Iinclude -DPAL_HOST_TEST tests/terminal.c tests/host_backend.c tests/services_host.c tests/kernel_host.c \
  tests/runtime_host.c tests/terminal_host.c "$root/host-terminal/release/libdotnet_pal_standalone.a" -Wl,--gc-sections -lpthread -ldl -lm -o artifacts/terminal/terminal-host
